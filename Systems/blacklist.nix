# /etc/nixos/SpidyNix/Systems/blacklist.nix
{ config, pkgs, lib, ... }:
{
  # ============================================================================
  # BLACKLISTED KERNEL MODULES
  # ============================================================================
  # Modules that will not be loaded by the kernel
  # ============================================================================
  boot.blacklistedKernelModules = [
    # ============================================================================
    # OBSCURE NETWORK PROTOCOLS
    # ============================================================================
    # Old, rare, or insecure network protocols
    # ============================================================================
    "af_802154"       # IEEE 802.15.4 (low-rate wireless personal area networks)
    "appletalk"       # AppleTalk (obsolete Apple networking protocol)
    "atm"             # Asynchronous Transfer Mode
    "ax25"            # Amateur X.25 (packet radio)
    "decnet"          # DECnet (DEC networking protocol)
    "econet"          # Econet (Acorn computers network)
    "ipx"             # Internetwork Packet Exchange (Novell NetWare)
    "n-hdlc"          # High-level Data Link Control
    "netrom"          # NetRom (amateur packet radio)
    "p8022"           # IEEE 802.3 (raw Ethernet)
    "p8023"           # Novell raw IEEE 802.3
    "psnap"           # SubNetwork Access Protocol
    "rds"             # Reliable Datagram Sockets
    "rose"            # ROSE packet radio protocol
    "tipc"            # Transparent Inter-Process Communication
    "x25"             # X.25 protocol

    # ============================================================================
    # OLD/RARE FILESYSTEMS
    # ============================================================================
    # Filesystems that are rarely used and potentially vulnerable
    # ============================================================================
    "adfs"            # Acorn Disc Filing System (Acorn computers)
    "affs"            # Amiga Fast File System
    "befs"            # Be File System (BeOS)
    "bfs"             # SCO UnixWare Boot File System
    "cramfs"          # Compressed ROM/RAM filesystem
    "efs"             # Extent File System (SGI IRIX)
    "erofs"           # Enhanced Read-Only File System
    "exofs"           # Extended Object File System
    "f2fs"            # Flash-Friendly File System (usually not needed on desktop)
    "freevxfs"        # Veritas filesystem driver
    "gfs2"            # Global File System 2 (Red Hat cluster filesystem)
    "hfs"             # Hierarchical File System (old Mac)
    "hfsplus"         # HFS Plus (modern Mac, but rarely needed on Linux)
    "hpfs"            # High Performance File System (OS/2)
    "jffs2"           # Journalling Flash File System v2 (embedded systems)
    "jfs"             # Journaled File System (IBM)
    "ksmbd"           # SMB3 Kernel Server
    "minix"           # Minix filesystem
    "nilfs2"          # New Implementation of Log-structured File System
    "omfs"            # Optimized MPEG Filesystem
    "qnx4"            # QNX4 filesystem
    "qnx6"            # QNX6 filesystem
    "squashfs"        # Compressed read-only filesystem (used by Snap, may want to keep if using Snap)
    "sysv"            # System V filesystem
    "udf"             # Universal Disk Format (optical media)

    # ============================================================================
    # SECURITY-SENSITIVE MODULES
    # ============================================================================
    # Modules that pose security risks or enable DMA attacks
    # ============================================================================
    "firewire-core"   # FireWire can enable DMA attacks
    "thunderbolt"     # Thunderbolt can enable DMA attacks (Thunderspy)
    "vivid"           # Virtual Video Test Driver (unnecessary for normal use)

    # ============================================================================
    # ADDITIONAL MODULES (Optional - uncomment if needed)
    # ============================================================================
    # Other modules you might want to blacklist
    # ============================================================================
    "pcspkr"           # PC speaker (annoying beeps)
    "snd_pcsp"         # PC speaker sound driver
    "iTCO_wdt"         # Intel TCO watchdog timer
    "nouveau"          # Open-source NVIDIA driver (if using proprietary)
    "radeon"           # Old AMD/ATI driver (if using amdgpu)
  ];
}

# ============================================================================
# KERNEL MODULE BLACKLIST
# ============================================================================
# This file blacklists kernel modules that are:
#   - Obscure/unused network protocols
#   - Rare/legacy filesystems
#   - Potential security risks
#   - Unnecessary for most systems
#
# Blacklisting reduces attack surface and prevents unused modules from loading
# ============================================================================

# WHAT IS BLACKLISTING?
# - Prevents specific kernel modules from loading
# - Reduces attack surface
# - Prevents conflicts between drivers
# - Saves memory and resources

# WHY BLACKLIST THESE MODULES?
# - Old protocols: Security vulnerabilities, no longer maintained
# - Rare filesystems: Potential bugs, rarely audited
# - DMA-capable devices: Can be used for physical attacks
# - Unused features: Waste resources, increase complexity

# NETWORK PROTOCOLS:
# - Most modern systems only need: TCP/IP, IPv6, Bluetooth
# - AppleTalk, IPX, etc. are obsolete
# - Amateur radio protocols rarely needed on desktop

# FILESYSTEMS:
# - Most systems only need: ext4, btrfs, xfs, vfat, ntfs
# - HFS/HFSPLUS: Only if dual-booting with macOS
# - Exotic filesystems: Rarely used outside specific contexts

# DMA ATTACKS:
# - FireWire and Thunderbolt allow direct memory access
# - Can bypass OS security to read/write RAM
# - Known attacks: Inception (FireWire), Thunderspy (Thunderbolt)
# - Blacklist if not using these interfaces

# SPECIAL CASES:
# - squashfs: Needed for Snap packages (Ubuntu)
# - hfsplus: Needed if accessing macOS drives
# - f2fs: Optimized for flash storage, may want for Android development
# - vivid: Only needed for video driver testing

# CHECKING LOADED MODULES:
# - List all: lsmod
# - Check specific: lsmod | grep MODULE_NAME
# - Module info: modinfo MODULE_NAME
# - Load manually: sudo modprobe MODULE_NAME

# TEMPORARILY BLACKLIST:
# To test before permanent blacklist:
#   sudo modprobe -r MODULE_NAME  # Unload
#   echo "blacklist MODULE_NAME" | sudo tee /etc/modprobe.d/blacklist.conf

# UNBLACKLIST:
# If you need a module, remove it from the list above and rebuild:
#   sudo nixos-rebuild switch

# TROUBLESHOOTING:
# - Device not working: Check if required module is blacklisted
# - Boot issues: May need to remove some blacklists
# - Firmware errors: Check dmesg for missing modules

# SECURITY CONSIDERATIONS:
# - Each module increases attack surface
# - Unused modules should be disabled
# - Balance security vs functionality
# - Regular security audits recommended

# PERFORMANCE:
# - Blacklisting reduces memory usage
# - Faster boot (fewer modules to initialize)
# - Less kernel code loaded
# - Minimal impact on running system

# CUSTOM BLACKLISTS:
# Add more modules to blacklist based on your needs:
# 1. Identify unused modules: lsmod
# 2. Research module purpose: modinfo MODULE_NAME
# 3. Add to list above
# 4. Test thoroughly after rebuild

# WHITELIST APPROACH:
# Alternative: Only load modules you need
# - More secure but more complex
# - Requires knowing all needed modules
# - Not practical for most users

# REFERENCES:
# - Kernel module documentation: /usr/src/linux/Documentation/
# - Security hardening guides: https://kernsec.org/wiki/
# - Common blacklists: https://github.com/Kicksecure/security-misc
# ============================================================================
